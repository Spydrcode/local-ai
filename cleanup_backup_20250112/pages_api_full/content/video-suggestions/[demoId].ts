...
This business serves ${detailedAnalysis.targetAudience} in ${detailedAnalysis.location}, specializing in ${detailedAnalysis.subNiche}. They differentiate themselves through: ${detailedAnalysis.differentiators.join(", ")}. Their brand voice is ${detailedAnalysis.brandVoice}, and they have ${detailedAnalysis.expertise}.`;
    ...
    const prompt = `Generate 15 short-form video content ideas for this business.

${businessInfo}

Create video concepts for TikTok, Instagram Reels, YouTube Shorts that are SPECIFIC to this business:
- Hook (first 3 seconds to grab attention - reference their specific sub-niche or differentiators)
- Concept (what happens in the video - show their actual services, processes, or unique approaches)
- Duration (15s, 30s, 60s)
- Platform recommendation
- Trending audio/format suggestion
- Call-to-action (specific to their business type and services)

Make each video idea reference:
...
{
  "videos": [
    { "hook": "...", "concept": "...", "duration": "...", "platform": "...", "trending_format": "...", "cta": "..." }
  ]
}`;
    ...
    return res.status(200).json({
      success: true,
      data: { videos: Array.isArray(videos) ? videos : [] },
    });
  } catch (err) {
    console.error("Video suggestions error:", err);
