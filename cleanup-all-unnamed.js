...
async function cleanupDatabase() {
  ...
  try {
    ...
    const fetchResponse = await fetch(`${SUPABASE_URL}/rest/v1/demos?select=id,business_name,website_url,created_at`, {
      headers: {
        'apikey': SUPABASE_KEY,
    ...
    const toDelete = allClients.filter(c => 
    ...
    toDelete.forEach(c => {
      console.log(`   - ${c.business_name} (${c.website_url}) - ${new Date(c.created_at).toLocaleDateString()}`);
    });
    ...
    for (const client of toDelete) {
      const deleteResponse = await fetch(`${SUPABASE_URL}/rest/v1/demos?id=eq.${client.id}`, {
        method: 'DELETE',
        headers: {
          'apikey': SUPABASE_KEY,
          'Authorization': `Bearer ${SUPABASE_KEY}`,
      ...
      if (!deleteResponse.ok) {
        console.error(`   ❌ Failed to delete ${client.business_name}: ${deleteResponse.statusText}`);
      } else {
    ...
    const finalResponse = await fetch(`${SUPABASE_URL}/rest/v1/demos?select=id,business_name,website_url,created_at`, {
      headers: {
        'apikey': SUPABASE_KEY,
        'Authorization': `Bearer ${SUPABASE_KEY}`,
        'Content-Type': 'application/json'
